...
static bool eos_cap_claim_slot(eos_cap_t cap, uint32_t cap_no,
                               eos_dev_t *owner_dev) {
...
#define EOS_CAP_ALLOC_FN(CAP_TYPE, CAP_ENUM, COUNT, RESERVED)                  \
  static const uint32_t eos_cap_##CAP_TYPE##_reserved[] = RESERVED;            \
  static bool eos_cap_alloc_##CAP_TYPE(uint32_t cap_no,                        \
                                       eos_dev_t *owner_dev) {                 \
    if (cap_no >= COUNT) {                                                     \
      eos_errno = EOS_ERR_CAP_NO_INVALID;                                      \
      return false;                                                            \
